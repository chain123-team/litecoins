...
const int LTSOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString LTSOIN_IPC_PREFIX("litecoins:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/litecoins-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/litecoins-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/litecoins-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        ...
        // If the litecoins: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(LTSOIN_IPC_PREFIX, Qt::CaseInsensitive)) // litecoins: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests) {
        ...
        if (!socket->waitForConnected(LTSOIN_IPC_CONNECT_TIMEOUT)) {
            delete socket;
            socket = nullptr;
            return false;
        }
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(LTSOIN_IPC_CONNECT_TIMEOUT);
                                                                       ...
                                                                       optionsModel(0)
{
    ...
    // on Mac: sent when you click litecoins: links
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start litecoins: click-to-pay handler"));
        } else {
...
//
// OSX-specific way of handling litecoins: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject* object, QEvent* event)
{
    // clicking on litecoins: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in litecoins: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(LTSOIN_IPC_PREFIX, Qt::CaseInsensitive)) // litecoins: URI
    {
        ...
        } else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient)) {
                ...
                emit message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid LiteCoins address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom lts addresses are not supported
