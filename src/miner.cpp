...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        if (fProofOfStake) {
            ...
            LogPrintf("LiteCoinsMiner : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LiteCoinsMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock))
        return error("LiteCoinsMiner : ProcessNewBlock, block not accepted");
...
// ***TODO*** that part changed in ltsoin, we are using a mix with old one here for now

void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    LogPrintf("LiteCoinsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litecoins-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running LiteCoinsMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
