...
/**
 * Fees smaller than this (in ults) are considered zero fee (for transaction creation)
 * We are ~100 times smaller then ltsoin now (2015-06-23), set minTxFee 10 times higher
 * so it's still 10 times lower comparing to ltsoin.
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 LTS.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-lts-address
                    CScript scriptChange;
