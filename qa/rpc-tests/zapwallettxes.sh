...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars LTSOIND and LTSOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

LTSOIND=${LTSOIND:-${1}/ltsoind}
CLI=${LTSOINCLI:-${1}/ltsoin-cli}
...
$LTSOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$LTSOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
