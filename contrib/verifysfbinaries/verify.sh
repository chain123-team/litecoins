...
###   This script attempts to download the signature file SHA256SUMS.asc from ltsoin.org
...
WORKINGDIR="/tmp/ltsoin"
TMPFILE="hashes.tmp"

#this URL is used if a version number is not specified as an argument to the script
SIGNATUREFILE="https://ltsoin.org/bin/0.9.2.1/SHA256SUMS.asc"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://ltsoin.org/bin/"
VERSIONPREFIX="ltsoin-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ltsoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[ltsoin-]<version>-[rc[0-9]] (example: ltsoin-0.9.2-rc1)"
   echo "wget output:"
