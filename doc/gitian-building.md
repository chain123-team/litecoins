...
*Setup instructions for a gitian build of LiteCoins using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LiteCoins
...
- [Building LiteCoins](#building-litecoins)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-ltsoin-host/README.md).
...
Clone the git repositories for litecoins and gitian and then checkout the litecoins version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the litecoins repository
...
To build LiteCoins (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the litecoins repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/litecoins/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/litecoins-official/litecoins
...
For example:
```bash
URL=https://github.com/konez2k/litecoins.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit litecoins=${COMMIT} --url litecoins=${URL} ../litecoins/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit litecoins=${COMMIT} --url litecoins=${URL} ../litecoins/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit litecoins=${COMMIT} --url litecoins=${URL} ../litecoins/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/litecoins-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/litecoins-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/litecoins-build.assert
```
...
[litecoins/gitian.sigs](https://github.com/litecoins/gitian.sigs/) repository, or if that's not possible to create a pull
request.
